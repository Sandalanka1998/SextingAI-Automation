describe('Chat History', ()=> {
    ...
    it('Functions', ()=> {
        ...
        cy.wait(2000)
        cy.get(':nth-child(3) > a > .v-row > .v-col-sm-3 > .profile > span').should('have.text', 'Testing') //AI parter name
        cy.wait(2000)
        cy.get('.chatlist-scroll > :nth-child(1)').click() //Select single chat
        cy.wait(1000)
        cy.get('.v-card-actions > .v-btn').click()
        cy.wait(1000)
        cy.get('.messagebox > .sender').should('have.text', "Alice") //Verify chat history display properly
